...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!prompt || !tags || !title || fullLyrics === undefined) {
      return NextResponse.json(
        { success: false, error: 'prompt, tags, title, and fullLyrics are required' },
        { status: 400 }
      );
    }
    ...
    const kieParams: KIEReplaceSectionRequest = {
      ...
      callBackUrl: `${process.env.CallBackURL}/api/replace-section-callback`,
