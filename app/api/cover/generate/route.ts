...
import { createCoverGeneration, updateCoverGeneration } from '@/features/lyrics-cover/lib/cover-db';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // musicTaskId: 用于存储到 cover_generations.music_task_id，callbacks/cover 据此更新 tracks
    ...
    const result = await musicApi.generateCover({ 
      taskId: taskIdForKIE,
      callBackUrl: `${process.env.CallBackURL}/api/callbacks/cover`
    });
