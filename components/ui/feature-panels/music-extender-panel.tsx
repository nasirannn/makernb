...
export const MusicExtenderPanel = (props: FeatureCreatePanelProps) => {
  ...
  const resolvedPanelTitle = panelTitle ?? t("studioFeatures.musicExtender");
  ...
  const applyExtendSourceTrack = React.useCallback((track: ExtendSourceTrack) => {
    ...
    if (!nextAudioUrl) {
      toast.error(t("toasts.selectedTrackHasNoAudioUrl"));
      return;
    }
    ...
    updateCurrentUploadState({
      coverFile: null,
      coverFileName: track.title || t("featurePanel.selectedTrack"),
  ...
  }, [clearMashupSelection, clearUploadCoverFile, updateCurrentUploadState, t]);
            ...
            {resolvedPanelTitle && (
              <div className="mb-3 px-1 space-y-1.5">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  {t("featurePanel.musicExtenderDescription")}
                </p>
              </div>
            )}
          </div>
