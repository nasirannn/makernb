...
import { StudioCustomModeContent, StudioSimpleModeContent, type AudioUploadIntent } from "@/components/ui/feature-panels/add-vocal-panel-mode-content";
...
const ADD_VOCAL_ALLOWED_MODELS: MusicModel[] = ["V5_5", "V5", "V4_5PLUS"];
...
export const AddVocalPanel = (props: FeatureCreatePanelProps) => {
  const {
    panelOpen,
  ...
  const addVocalModelOptions = React.useMemo(
    () => modelOptions.filter((option) => ADD_VOCAL_ALLOWED_MODELS.includes(option.value)),
    []
  );
  ...
  const handleModelSelect = React.useCallback((model: MusicModel) => {
    const selectedOption = addVocalModelOptions.find((option) => option.value === model);
  ...
  }, [addVocalModelOptions, canUseV5Model, updateSelectedModel]); // 故意移除所有状态依赖，避免循环更新 - 函数内部已经有最新的状态引用
