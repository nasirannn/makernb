...
import { StudioCustomModeContent, StudioSimpleModeContent, type AudioUploadIntent } from "@/components/ui/feature-panels/music-cover-panel-mode-content";
...
export const MusicCoverPanel = (props: FeatureCreatePanelProps) => {
  const {
    panelOpen,
  ...
  const canControlPublicVisibility = hasPermission('control_public_visibility');
  ...
  const handlePublicVisibilityChange = React.useCallback((nextPublished: boolean) => {
    if (nextPublished === isPublished) return;
    if (!nextPublished && !canControlPublicVisibility) {
      setIsPricingOpen(true);
      return;
    }
    setIsPublished?.(nextPublished);
  }, [canControlPublicVisibility, isPublished, setIsPublished]);
    ...
    >
            ...
            {resolvedPanelTitle && (
              <div className="mb-3 px-1 space-y-1.5">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  {t("featurePanel.musicCoverDescription")}
                </p>
                {showModeTabs && (
      ...
      {mode === "simple" ? (
        ...
        <StudioCustomModeContent
          ...
          preferTrackUploadCard={requiresTrackUpload}
          ...
          canDisablePublicVisibility={canControlPublicVisibility}
        />
      )}
          </div>
