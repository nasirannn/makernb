...
export const MashupPanel = (props: FeatureCreatePanelProps) => {
  ...
  const syncMashupTracksFromSlots = React.useCallback((slots: [MashupSelectionSlot, MashupSelectionSlot]) => {
    ...
    const nextTracks: MashupPreviewTrack[] = slots.map((slot, index) => {
      ...
      const fallbackName = file.name.replace(/\.[^/.]+$/, "") || `Mashup Audio ${index + 1}`;
      return {
        file,
    ...
    stopMashupPlayback(true);
  }, [clearMashupPreviewTracks, stopMashupPlayback]);
  ...
  }, [allowMashupAction, user, setIsAuthModalOpen, canUseMashup, uploadCoverFile]);
  ...
  const handleMashupSlotPlayPause = React.useCallback(async (slotIndex: 0 | 1) => {
    ...
    try {
      ...
      setMashupPlayingIndex(null);
      toast.error("Unable to play selected audio.");
    }
  }, [getMashupSlotPlaybackAudioUrl, getOrCreateMashupAudio, mashupPlayingIndex]);
  ...
  const handleMashupSlotFileChange = React.useCallback(async (
    ...
    const fileName = file.name.replace(/\.[^/.]+$/, "") || `Mashup Audio ${slotIndex + 1}`;
  ...
  }, [maxDirectUploadBytes, readAudioDuration]);
            ...
            {panelTitle && (
              <div className="mb-3 px-1 space-y-1.5">
                ...
                <p className="mt-1 text-xs md:text-sm text-muted-foreground">
                  Blend two tracks into one seamless mashup.
                </p>
              </div>
            )}
          </div>
